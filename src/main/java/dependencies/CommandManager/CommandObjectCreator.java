        ...
        switch (command) {
            ...
            case "login":
                if (commandArgs.size() >= 2) {
                    return new CommandObject(command, commandArgs.get(0), commandArgs.get(1));
                } else {
                    return createErrorObject("Error", String.format("Not enough arguments: 2 is required, %s got", commandArgs.size()));
                }
